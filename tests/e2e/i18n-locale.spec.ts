...
// Note: locale routing & switcher tests require the preview deployment
// (production still has old middleware until this PR is merged & deployed).
...
const isPreview = !CUSTOMER_BASE.includes('cutebunnyrental.com');
...
if (isPreview) {
  ...
  // Production fallback: verify existing Thai-only redirect behavior
  test.describe('i18n — customer locale redirect (BUG-544, production)', () => {
    test('/en/products → redirect to /th/products', async ({ request }) => {
      const res = await request.get(`${CUSTOMER_BASE}/en/products`, {
        maxRedirects: 0,
      });
    ...
    test('/zh/products → redirect to /th/products', async ({ request }) => {
      const res = await request.get(`${CUSTOMER_BASE}/zh/products`, {
        maxRedirects: 0,
      });
      expect([301, 302, 307, 308]).toContain(res.status());
  ...
  test.describe('i18n — customer Thai-only mode (BUG-544, production)', () => {
